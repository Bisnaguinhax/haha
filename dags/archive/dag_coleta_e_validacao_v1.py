# ===================================================================================
# DAG DE COLETA DE DADOS EXTERNOS COM VALIDAÇÃO - VERSÃO CORRIGIDA
# ===================================================================================
...
# Define o caminho base dentro do contêiner do Airflow
# Este caminho é mapeado para a pasta local pelo docker-compose.yml
...
def _get_api_key_from_vault(**kwargs):
    ...
    # Esta lógica deve estar DENTRO da função para ser executada pela tarefa
    SECRET_KEY = os.getenv('SECURITY_VAULT_SECRET_KEY')
    if not SECRET_KEY:
        raise ValueError("ERRO CRÍTICO: A variável de ambiente 'SECURITY_VAULT_SECRET_KEY' não está definida.")

    # O caminho para o vault deve ser o caminho DENTRO do contêiner
    VAULT_DB_PATH = f'{BASE_DATA_PATH}/security_vault.db'
...
def _coleta_ipca():
    ...
    indicadores_path = f'{BASE_DATA_PATH}/indicadores'
    os.makedirs(indicadores_path, exist_ok=True)
    ...
    df_ipca.to_csv(f"{indicadores_path}/ipca_coletado.csv", index=False)
...
def _coleta_clima(**kwargs):
    ...
    clima_path = f'{BASE_DATA_PATH}/clima'
    ...
    df_clima.to_csv(f"{clima_path}/clima_coletado.csv", index=False)
...
def _verifica_anomalias():
    ...
    ipca_path = f'{BASE_DATA_PATH}/indicadores/ipca_coletado.csv'
    clima_path = f'{BASE_DATA_PATH}/clima/clima_coletado.csv'
